...
export async function signinMember(req, res) {
    try {
        ...
        bcrypt.compare(req.body.password, member.password).then(response => {
            ...
            let authToken = jwt.sign({ email: member.email, memberid: member.memberid },
                process.env.AUTH_KEY, { expiresIn: "1h" });
            return res.status(200).json({
                status: true,
                message: "Members authentication successful",
                member: { name: member.name, email: member.email, memberid: member.memberid },
                token: authToken,
                expiresIn: 3600
            })
        })
...
export async function viewMember(req, res) {
    try {
        let allmembers = await Members.findAll({where: {memberid: req.params.id}});
        if (allmembers) {
            res.json({
                success: true,
